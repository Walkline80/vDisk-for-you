...
public class RenamePopupScreen extends PopupScreen implements vDiskSDKResource
{
	...
	public RenamePopupScreen(String oldName)
	{
		super(new VerticalFieldManager());

		_labelTitle = new LabelField(getResString(RENAME_RENAME_OBJECT), Field.FIELD_HCENTER);
		_labelTitle.setMargin(0,  0, 5, 0);
		_blockField=new EditField(getResString(RENAME_LABEL_NAME), oldName, 200, EditField.NO_NEWLINE);
		_labelNotice = new LabelField(null, LabelField.NON_FOCUSABLE);

		_buttonOk = new ButtonField(getResString(RENAME_BUTTON_OK), ButtonField.CONSUME_CLICK | ButtonField.NEVER_DIRTY);
		...
		ButtonField buttonCancel = new ButtonField(getResString(RENAME_BUTTON_CANCEL), ButtonField.CONSUME_CLICK | ButtonField.NEVER_DIRTY);
